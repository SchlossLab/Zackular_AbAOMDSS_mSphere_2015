...
similarity and tumor burden (\rho=`r round(dist_tumor_cor$estimate, 2)`,
P=`r round(dist_tumor_cor$p.value, 2)`; Figure 4).
...
```{r figure4, fig.width=3.75, fig.height=5}

pdf(file="test.pdf", width=3.75, height=7)
design <- matrix(c(2,1,
                    5,3,
                    5,4,
                    0,6), byrow=T, nrow=4)
z <- layout(design, width=c(0.1,1), height=c(1.5,1,1,0.2))
...
text(x=0.15, y=0.65, label="Number of tumors", srt=90)
...
par(mar=c(0,0,0,0))
plot(NA, xlim=c(0,1), ylim=c(0,1), axes=F, xlab="", ylab="", xpd=TRUE)
text(x=0.15, y=0.5, label="Relative abundance (%)", srt=90)


plot(NA, xlim=c(0.1,1), ylim=c(0,1), axes=F, xlab="", ylab="")
text(x=0.5, y=0.25, label="Days since AOM administration")
